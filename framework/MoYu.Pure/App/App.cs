...
public static class App
{
    ...
    private static (IEnumerable<Assembly> Assemblies, IEnumerable<Assembly> ExternalAssemblies, IEnumerable<string> PathOfExternalAssemblies) GetAssemblies()
    {
        ...
        if (!string.IsNullOrWhiteSpace(entryAssembly.Location))
        {
            ...
            // 读取项目程序集或 MoYu 官方发布的包，或手动添加引用的dll，或配置特定的包前缀
            scanAssemblies = dependencyContext.RuntimeLibraries
               .Where(u =>
                      (u.Type == "project" && !excludeAssemblyNames.Any(j => u.Name.EndsWith(j))) ||
                      (u.Type == "package" && (u.Name.StartsWith(nameof(MoYu)) || supportPackageNamePrefixs.Any(p => u.Name.StartsWith(p) && u.RuntimeAssemblyGroups.Count > 0))) ||
        ...
        else
        {
            ...
            else
            {
                ...
                Console.WriteLine(TP.Wrapper("Deploy Console"
                    ...
                    , "##Documentation## https://MoYu.net/docs/singlefile"));
                Console.ResetColor();
            }

            // 通过 AppDomain.CurrentDomain 扫描，默认为延迟加载，正常只能扫描到 MoYu 和 入口程序集（启动层）
