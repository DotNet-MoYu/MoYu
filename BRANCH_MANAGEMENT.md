...
`MoYu` 项目采用了 [GitFlow](https://www.atlassian.com/git/tutorials/comparing-workflows/gitflow-workflow) 工作流进行分支管理，这是一种专门为大型项目设计的长期支持性分支模型。其核心理念是分离出主干分支（`main` 或 `master`）、开发分支 (`develop`)、特性分支 (`feature/*`)、发布分支 (`release/*`) 和热修复分支 (`hotfix/*`)。
...
感谢您的配合，遵循上述分支管理策略将使 `MoYu` 项目的开发更加有序、高效！
