...
`├──` **`assets`** — 仓库静态资源<br>
`├──` **`clients`** — 客户端工具目录<br>
`├──` **`framework`** — `MoYu` 框架源码<br>
`├──` **`references`** — `MoYu` API 源码<br>
`├──` **`samples`** — `MoYu` 框架演示例子<br>
`├──` **`schemas`** — `MoYu` 配置文件 `JSON Schema`<br>
...
`├──` **`LICENSE-APACHE`** — `MoYu` Apache-2.0 开源协议<br>
`├──` **`LICENSE-MIT`** — `MoYu` 仓库 MIT 开源协议<br>
`├──` **`README.md`** — `MoYu` 国际版仓库介绍<br>
`├──` **`README.zh.md`** — `MoYu` 中国版仓库介绍<br>
