...
using MoYu.Application.Services;
...
public class PersonService : IDynamicApiController
{
    ...
    /// </summary>
    /// <remarks>
    /// # 先知 / MoYu ([探索版](https://gitee.com/dotnetchina/MoYu/tree/experimental/))
    ///
    ...
    /// ## 安装 / Installation
    ///
    /// - [Package Manager] (https://www.nuget.org/packages/MoYu)
    ///
    /// ```powershell
    /// Install-Package MoYu
    /// ```
    ///
    /// - [.NET CLI] (https://www.nuget.org/packages/MoYu)
    ///
    /// ```powershell
    ...
    ///
    /// We have several examples [on the website] (https://MoYu.net). Here is the first one to get you started:
    ///
    /// ```cs
    /// Serve.Run();
    ///
    ...
    /// ## 文档 / Documentation
    ///
    /// 您可以在[主页] (https://MoYu.net)或[备份主页](https://MoYu.net)找到 MoYu 文档。
    ///
    /// You can find the MoYu documentation[on the website](https://MoYu.net) or [on the backup website](https://MoYu.net).
    ///
    /// ## 贡献 / Contributing
    ///
    /// 该存储库的主要目的是继续发展 MoYu 核心，使其更快、更易于使用。 MoYu 的开发在[Gitee](https://gitee.com/dotnetchina/MoYu) 上公开进行，我们感谢社区贡献错误修复和改进。
    ///
    /// 阅读[贡献指南] (https://MoYu.net/docs/contribute)内容，了解如何参与改进 MoYu。
    ///
    /// The main purpose of this repository is to continue evolving MoYu core, making it faster and easier to use.Development of MoYu happens in the open on[Gitee] (https://gitee.com/dotnetchina/MoYu), and we are grateful to the community for contributing bugfixes and improvements.
    ///
    /// Read[contribution documents] (https://MoYu.net/docs/contribute) to learn how you can take part in improving MoYu.
    ///
    /// ## 许可证 / License
    ///
    /// MoYu 采用[MIT](https://gitee.com/dotnetchina/MoYu/blob/v4/LICENSE) 开源许可证。
    ///
    /// MoYu uses the[MIT] (https://gitee.com/dotnetchina/MoYu/blob/v4/LICENSE) open source license.
    ///
    /// ```
    ...
    ///             https://gitee.com/dotnetchina/MoYu/blob/v4/LICENSE
    ...
    /// # MoYu 探索版
    ///
    /// > 在过去一年，实现 `MoYu` 从无到有，编写文档已逾百万字，过程心酸开源人自知。
    /// >
    ...
    /// >
    /// > 所以，这一次重新起航，重塑 `MoYu` 重塑自己。也许未来在某个 IT 圈但凡有人谈起 `.NET` 还能瞟到 `MoYu` 的身影。
    ///
    /// ---
    ///
    /// 🎉 探索 MoYu 未来更多可能性，实现无第三方依赖的版本，所有模块功能按需安装，按需加载。
    ///
    ...
    /// ## 包说明
    ///
    /// - `MoYu.Core`：无第三方依赖，可在 `.NET 6` 所有项目类型中运行。
    /// - `MoYu`：内部依赖 `MoYu.Core` 且无第三方依赖，**聚焦 `Web` 应用**，采用共享框架 `<FrameworkReference Include="Microsoft.AspNetCore.App" />` 模式
    ///  </remarks>
    ...
    public async Task 测试分表插入()
    {
        await _personRepository
           .InsertFromSegmentsAsync(table => [table], new Person { Id = 2, Name = "MoYu", Age = 32 });
    }

    public async Task 测试分表更新()
    {
        await _personRepository
           .UpdateFromSegmentsAsync(table => [table], new Person { Id = 1, Name = "MoYu", Age = 32 });
    }
