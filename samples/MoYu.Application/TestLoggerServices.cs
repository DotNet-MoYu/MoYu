...
public class TestLoggerServices : IDynamicApiController
{
    ...
    public void 测试日志()
    {
        using var scope = _logger.ScopeContext(ctx => ctx.Set("Name", "MoYu"));
    ...
    public void 测试日志上下文()
    {
        "设置日志上下文".ScopeContext(ctx => ctx.Set("name", "MoYu")).LogWarning();
        var (logger, scoped) = Log.ScopeContext(ctx => ctx.Set("name", "MoYu"));
